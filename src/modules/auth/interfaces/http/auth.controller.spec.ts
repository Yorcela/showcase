...
describe("AuthController", () => {
  ...
  describe("refresh", () => {
    it("should refresh tokens and rotate cookie", async () => {
      const res = createResponse();
      const auth = AuthEntity.fromRaw({
        accessToken: "new-access",
        refreshToken: "new-refresh",
        sessionId: "ses_1" as Cuid2,
        refreshTokenExpiresAt: Date.now() + 100_000,
      });
    ...
    it("should not reset cookie when token not rotated", async () => {
      const res = createResponse();
      const auth = AuthEntity.fromRaw({
        accessToken: "new-access",
      ...
      authUseCase.refreshFromToken.mockResolvedValue({ authEntity: auth, rotated: false });
