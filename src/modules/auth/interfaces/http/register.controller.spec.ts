...
import { AuthTokenPayloadDto } from "../presenters/dto/login.dto";
import { AuthRegisterInputDto, AuthVerifyEmailInputDto } from "../presenters/dto/register.dto";
...
describe("RegistrationController", () => {
  ...
  describe("verifyEmail", () => {
    it("should create auth tokens when verification succeeds", async () => {
      ...
      registerUseCase.verifyEmail.mockResolvedValue(verification);
      authUseCase.createAuthTokenForUser.mockResolvedValue(auth);

      const res = createResponse();
      const result = await controller.verifyEmail(input, res as Response);

      expect(registerUseCase.verifyEmail).toHaveBeenCalledWith(input);
      expect(authUseCase.createAuthTokenForUser).toHaveBeenCalledWith(verification.userId);
      expect(res.cookie).toHaveBeenCalled();
      expect(result).toBeInstanceOf(AuthRegisterEmailVerifiedSuccess);
      expect(result.code).toBe(AuthSuccessCode.REGISTRATION_EMAIL_VERIFIED);
    ...
    it("should throw when verification token is invalid", async () => {
      ...
      registerUseCase.verifyEmail.mockRejectedValue(
        new AuthRegisterVerificationCodeInvalidOrExpiredError(),
      );

      await expect(
        controller.verifyEmail(input, createResponse() as Response),
      ).rejects.toBeInstanceOf(AuthRegisterVerificationCodeInvalidOrExpiredError);
      expect(authUseCase.createAuthTokenForUser).not.toHaveBeenCalled();
    });
  });
});
